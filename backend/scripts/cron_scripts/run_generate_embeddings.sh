#!/bin/bash
# Wrapper script for generate_embeddings.py to be used with cron
...
if [ ! -f "$PYTHON_BIN" ]; then
    echo "Error: Virtual environment Python not found at $PYTHON_BIN" >> "$BACKEND_DIR/logs/cron_generate_embeddings.log" 2>&1
    exit 1
fi
...
"$PYTHON_BIN" "$SCRIPTS_DIR/generate_embeddings.py" >> "$BACKEND_DIR/logs/cron_generate_embeddings.log" 2>&1
